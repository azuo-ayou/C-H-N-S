function error = Hy_norm_2D(solution,exact_der_solution_fun,exact_solution_fun,Tb,T,P,basis_type,t)
...
for i=1:Gpn
   ...
   %int_value=int_value+Gauss_weight(i)*(feval(exact_solution_fun,Gauss_point(i))-FE_function_1D(Gauss_point(i),uh_local,vertices,basis_type,der))^2; 
    sumHx=sumHx+uh_local(j)*FE_local_basis_2D(Gauss_point(1,i),Gauss_point(2,i),vertices,basis_type,j,0,1);
    sum=sum+uh_local(j)*FE_local_basis_2D(Gauss_point(1,i),Gauss_point(2,i),vertices,basis_type,j,0,0);
    end
    int_value_Hy=int_value_Hy+Gauss_weight(i)*(feval(exact_der_solution_fun,Gauss_point(1,i),Gauss_point(2,i),t)-sumHx)^2;
    %int_value=int_value+Gauss_weight(i)*(feval(exact_solution_fun,Gauss_point(1,i),Gauss_point(2,i),t)-sum)^2;
    
end
local_Hx_error=int_value+int_value_Hy;
end