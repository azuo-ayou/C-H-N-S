function A=J_and_phi_add_xx(Exact_J,matrix_size,Pb,Tb,basis_type_trial,basis_type_test)
...
%     uh_local_ex=solution_n1_ex(Tb(:,n));
    
    for alpha = 1 : number_of_local_basis
        for beta = 1 : number_of_local_basis 
            int_value =Gauss_vol_int_trial_test_AN(Exact_J,Gauss_weight,Gauss_point,vertices,basis_type_trial,alpha,basis_type_test,beta,Gpn);
            A(Tb(beta,n),Tb(alpha,n))= A(Tb(beta,n),Tb(alpha,n))+int_value;           
        end        
    end 
end

function int_value = Gauss_vol_int_trial_test_AN(Exact_J,Gauss_weight,Gauss_point,vertices,basis_type_trial,basis_index_trial,basis_type_test,basis_index_test,Gpn)
int_value=0;
for k= 1 : Gpn
    int_value1=0;
    for i = 1 : Gpn
%         int_value=int_value+Gauss_weight(i)*feval(Exact_J,Gauss_point(1,k)-Gauss_point(1,i),Gauss_point(2,k)-Gauss_point(2,i))*FE_local_basis_2D(Gauss_point(1,i),Gauss_point(2,i),vertices,basis_type_trial,basis_index_trial,0,0)* FE_local_basis_2D(Gauss_point(1,i),Gauss_point(2,i),vertices,basis_type_test,basis_index_test,0,0);
        ...
        int_value1=int_value1+Gauss_weight(i)*feval(Exact_J,Gauss_point(1,k)-Gauss_point(1,i),Gauss_point(2,k)-Gauss_point(2,i));
    end
    int_value=int_value+int_value1*Gauss_weight(k)*FE_local_basis_2D(Gauss_point(1,k),Gauss_point(2,k),vertices,basis_type_test,basis_index_test,0,0)*FE_local_basis_2D(Gauss_point(1,k),Gauss_point(2,k),vertices,basis_type_trial,basis_index_trial,0,0);
...
%     int_value=int_value+Gauss_weight(i)*(c1/2+c2/2)^2*FE_local_basis_2D(Gauss_point(1,i),Gauss_point(2,i),vertices,basis_type_trial,basis_index_trial,der_trial_x,der_trial_y)* FE_local_basis_2D(Gauss_point(1,i),Gauss_point(2,i),vertices,basis_type_test,basis_index_test,der_test_x,der_test_y);
end