function b = assemble_E_3_2D_local(solution_n1,Pb,Tb,basis_type,ef,gram)% ||grid(phi_n+1)||^2
b=0;
ef_1=1;
Gpn=9;
...
%     uh_local2=solution_n1_ex(Tb(:,n));
   for i = 1 : Gpn
    ...
    c1=c1+uh_local1(j)*FE_local_basis_2D(Gauss_point(1,i),Gauss_point(2,i),vertices,basis_type,j,1,0);
     c2=c2+uh_local1(j)*FE_local_basis_2D(Gauss_point(1,i),Gauss_point(2,i),vertices,basis_type,j,0,1);
     c3=c3+uh_local1(j)*FE_local_basis_2D(Gauss_point(1,i),Gauss_point(2,i),vertices,basis_type,j,0,0);
    end
%     c3 = (3*c1-c2)/2;
    ...
    int_value = int_value+Gauss_weight(i)*(gram*ef^2*(c1^2+c2^2)/2+gram*(c3^2-1)^2/4)/ef_1^2;
   end
   b = b + int_value;
end
% b = sqrt(b);
